...
class ExtGrid {
    ...
    JSONArray getFields() {
        ...
        arrAttribute?.each {
            jSONArray.put(MyStringUtils.toDataIndex(it))
        }
        return jSONArray
    }
    ...
    JSONArray getColumns() {
        ...
        for (int i = 0; i < arrHead.size(); i++) {
            ...
            oneColumn.put("dataIndex", MyStringUtils.toDataIndex(attr))
    ...
    ExtGrid(boolean hasRowNumber) {
        this.hasRowNumber = hasRowNumber
    }

    ExtGrid(boolean hasRowNumber, boolean isFlex, String url, String renderTo, String createUrl) {
        this.hasRowNumber = hasRowNumber
        this.isFlex = isFlex
        this.url = url
    ...
    JSONObject toJSON() {
        ...
        jSONObject.put("renderTo", renderTo)
        jSONObject.put("createUrl", createUrl)
        return jSONObject
    }
